...
namespace Ranger.RabbitMQ
{
    public class CorrelationContext : ICorrelationContext
    {
        ...
        [JsonConstructor]
        private CorrelationContext(Guid correlationContextId, string domain, string userEmail, Guid resourceId, string traceId, string spanContext, string connectionId, string name, string origin, string culture, string resource, int retries)
        {
            CorrelationContextId = correlationContextId;
            Domain = domain;
            UserEmail = userEmail;
        ...
        {
            return Create<T>(context.CorrelationContextId, context.Domain, context.UserEmail, context.ResourceId, context.TraceId, context.ConnectionId, context.Origin, context.Culture, context.Resource);
        }

        public static ICorrelationContext Create<T>(Guid correlationContextId, string domain, string userEmail, Guid resourceId, string origin, string traceId, string spanContext, string connectionId, string culture, string resource = "")
        {
            return new CorrelationContext(correlationContextId, domain, userEmail, resourceId, traceId, spanContext, connectionId, typeof(T).Name, origin, culture, resource, 0);
        }
